...
# ---------- Download and transform SS payments data ----------

# Create directory
suppressWarnings(dir.create("raw-data/dss"))

pg <- read_html("https://data.gov.au/data/dataset/cff2ae8a-55e4-47db-a66d-e177fe0ac6a0")
...
links <- html_attr(html_nodes(pg, "a"), "href") %>%
  ...
  filter(str_detect(filename, "demographic|demogrphics")) %>%
  # Fix anamalous filename for Sept 2015 (201509)
  mutate(filename = str_replace_all(filename, "201509", "september-2015")) %>%
  # Extract month and year
  mutate(month = str_sub(str_extract(filename, "march|mar|june|september|sept|december|dec"), 1, 3),
